...
def search_research(request):
    ...
    # Apply date filtering if provided (ensure this logic exists correctly)
    if start_date:
    ...
    # Order results consistently after all filtering
    projects = projects_query.order_by("-date_presented")

    # Define the full context for the template
    context = {
        "projects": projects,
        "query": query,
